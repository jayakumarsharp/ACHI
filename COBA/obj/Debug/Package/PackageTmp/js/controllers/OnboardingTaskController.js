ReportApp.controller('OnboardingTaskController', function ($scope, $rootScope, OnboardingTaskService, $timeout, Upload, $window) {
    ...
    $scope.GetCurrencyConversionForId = function (id) {
        ...
        OnboardingTaskService.GetAllTaskById(id).success(function (data) {
            $scope.editMode = true;
            $scope.downloadurl += data[0].StrategyNumber + "\\";
    ...
    $scope.upload = function () {
        if ($scope.upload_form.file.$valid && $scope.file) { //check if from is valid
            ...
            Upload.upload({
                ...
                data: { Id: $scope.ecurrency.Id, TaskComments: $scope.ecurrency.TaskComments, StrategyNumber: $scope.ecurrency.StrategyNumber, ExisitingFiles: $scope.ecurrency.TaskAttachment, obj: $scope.file }//pass file as data, should be user ng-model
